...
const updateDiamond = asyncHandler(async (req, res, next) => {
  console.log(`📝 Updating Diamond [${req.params.id}]:`, Object.keys(req.body));
  try {
    ...
    if (!diamond) {
      res.status(404);
      return next(new Error("Diamond not found"));
    }
    console.log("✅ Diamond updated successfully:", req.params.id);
    res.json(diamond);
  } catch (error) {
    console.error("❌ Update Diamond Error:", error.message);
    res.status(500);
    return next(error);
  }
});
...
const updateDiamondStatus = asyncHandler(async (req, res, next) => {
  const { availability } = req.body;
  const diamondId = req.params.id;
  console.log(`📝 Updating Diamond Status [${diamondId}] to:`, availability);

  if (!availability) {
    res.status(400);
    return next(new Error("Availability status is required."));
  }
  try {
    ...
    if (diamond.user.toString() !== req.user._id.toString()) {
      res.status(403);
      return next(new Error("User not authorized to update this diamond."));
    }
    diamond.availability = availability;
    const updatedDiamond = await diamond.save();
    console.log("✅ Diamond status updated successfully:", diamondId);
    res.status(200).json({ success: true, diamond: updatedDiamond });
  } catch (error) {
    console.error("❌ Update Diamond Status Error:", error.message);
    res.status(500);
    return next(error);
  }
});
